...
## Testing Guidelines
Add table-driven Go tests in `_test.go` files beside the code they cover. Prioritize unit tests for pure helpers such as event filtering, timestamp selection, health handling, and config loading behavior where it can be isolated cleanly. Validate chart changes with `helm lint chart`. When adding behavior that affects output, flags, metrics, or health endpoints, include at least one regression test or a documented manual verification step in the PR.
